...
const getMinAge = (data) => {
	return data.map(passenger => passenger.fields.age).sort((a,b) => a-b)[0]
}
...
const getMaxAge = (data) => {
	// return data.map(passenger => passenger.fields.age).sort((a,b) => b-a)[0]
	const ages = data.map(passenger => passenger.fields.age).filter(age => age !== undefined)
...
const getMinFare = (data) => {
	return data.map(passenger => passenger.fields.fare).sort((a,b) => a-b)[0]
}
...
const getMaxFare = (data) => {
	return data.map(passenger => passenger.fields.fare).sort((a,b) => b-a)[0]
}
...
const getTotalFare = (data) => {
	return data.map(passenger => passenger.fields.fare).reduce((a,b) => a+b ,0)
...
const getAverageFare = (data) => {
	return data.map(passenger => passenger.fields.fare).reduce((a,b) => a+b ,0) / data.length
}
...
const getMedianFare = (data) => {
	return data.map(passenger => passenger.fields.fare).sort((a,b) => a-b)[Math.floor(data.length/2)]
}
...
const getAverageAge = (data) => {
	const validAges = data.filter(passenger => passenger.fields.age !== undefined);
  const sumOfAges = validAges.reduce((sum, passenger) => sum + passenger.fields.age, 0);
  return sumOfAges / validAges.length;
}
...
const getMedianAge = (data) => {
  ...
  const middleIndex = Math.floor(validAges.length / 2);

  return validAges.length % 2 === 0
    ? (validAges[middleIndex - 1] + validAges[middleIndex]) / 2
    : validAges[middleIndex];
}
...
const getAverageAgeByGender = (data, gender) => {
  ...
  if (validAges.length === 0) {
    return 0; // Return 0 if there are no valid ages for the specified gender
  }

  const sumOfAges = validAges.reduce((sum, age) => sum + age, 0);
  return sumOfAges / validAges.length;
};
